...
runtime, and the boss and the arena's flagstone floor are models that scripts build with
Blender (see [Content pipeline](#content-pipeline-python)). `scenes/main.tscn` is just a root node with
`scripts/main.gd`.
...
shaders/                   night sky, flagstones, mortar bed, puddle, the earth round the plaza
data/                      rigs.json, animations.json, models.json (generated, see below)
models/                    boss.glb, boss_staff.glb + ribbon textures, arena_floor.glb (generated with Blender)
audio/, textures/, fonts/  generated sound effects, brush kanji, floor textures, UI font (OFL)
tools/                     Python content pipeline + previewers (ignored by Godot)
  model3d/                 scripted Blender modelling: mesh builders, pattern and floor textures, baking
```
...
or the floor also needs Blender as a Python module: `pip install bpy==4.5.9` (Blender 4.5 LTS,
Python 3.11), and the floor needs `shapely`.
